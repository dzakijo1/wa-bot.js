...
const {wallpaperanime, quotes, anime, simsimichat, nhentai, corona, brainly, wait, surat} =  require('./lib/functions')
...
let {prefix, banChats, restartState: isRestart,mtc: mtcState, whitelist ,sAdmin} = setting
const serverOption = {
    headless: true,
    qrRefreshS: 20,
    qrTimeout: 0,
...
Lihat gambar caption ku :)`;
                    ...
                    case prefix+'limit':
                        ...
                        for(let lmt of limit){
                            if(lmt.id === serial){
                                reply(`Sisa limit media anda tersisa : *${25-lmt.limit}*`)
                                found = true
                            }
                        }
                        if (found === false){
                            ...
                            reply(`Sisa limit media anda tersisa : *25*`)
                        }
                        break
                    case '#bot refresh':
                        if(!isSadmin) return
